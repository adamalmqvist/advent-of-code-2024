...
const findTrails = (i: number, j: number, target: number, trail: string) => {
    ...
    if (target === 9) {
        ...
        for (const position of positions) {
            posSet.add(trail + JSON.stringify(position));
        }
        return positions;
    }
    for (const position of positions) {
...
for (let i = 0; i < grid.length; i++) {
    for (let j = 0; j < grid[i].length; j++) {
        if (grid[i][j] === 0) {
            findTrails(i, j, 1, JSON.stringify([i, j]));
