...
for (let j = 0; j < updates.length; j++) {
  ...
  for (let i = 0; i < update.length; i++) {
    ...
    const requiredFilesAfterFile = rulesForFile.map((rule) => rule[1]).filter((rule) => file !== rule)
    const requiredFilesBeforeFile = rulesForFile.map((rule) => rule[0]).filter((rule) => file !== rule)
